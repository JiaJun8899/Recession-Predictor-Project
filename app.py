...
def Addrect(fig):
    #this function is to add yellow blocks to show recession in the graph
...
def datasets():
    ...
    dwngraph = thedata
    #if statments to check and then generate the proper data and sending the JSON string to plotly to plot in javascript
    if thedata == "cpi":
    ...
    if thedata=="multiple":
        Message=" "
        #changing of data if the requested data is the difference for each month
        if 'diff' in Diff:
        ...
        #apply the date filters that the user requested
